...
def updateName(canvas, name):
    canvas.delete("all")
    canvas.create_text(250, 250, text=name, font=("Arial", 50))
    ...
    try:
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                # Update the recognized name if confidence is above a threshold
                if confidence < 75:  # Confidence threshold, you can adjust this
                    updateName(canvas, "Stranger Danger")
                    print(f"Recognized: {data.get(label)} with confidence: {confidence}")
